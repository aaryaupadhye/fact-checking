...
def predict_class(model, inputs):
        ...
        # Apply sigmoid to get a set of probabilities for binary classification
        probabilities = F.sigmoid(logits)
                
        for probs in probabilities:
            
            # Classify with respect to labels of finetuned model
            if (probs[1] > probs[0]):
...
def main():
    ...
    model, tokenizer = utils.load_model(model_name="../../finetuned_models/roberta/")
    ...
    # Visualization of hops analysis
    fig = utils.plot_hops_analysis(correct_predictions, wrong_predictions)    
    fig.savefig("output/hops_finetuned.png")
